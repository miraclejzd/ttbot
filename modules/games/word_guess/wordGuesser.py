...
class WordGuesser(object):
    ...
    def __init__(
        ...
        self.font = ImageFont.truetype(
            str(Path(os.getcwd()) / "data" / "fonts" / "KarnakPro-Bold.ttf"),
            font_size,
        )
    ...
    def draw(self, word: str) -> "WordGuesser":
    ...
    def get_color(self, w: str) -> List[Tuple[str, Tuple[int, int, int]]]:
        ...
        cnt_set = {chr(i): 0 for i in range(97, 123)}
        cnt_set['-'] = 0
        for i, char in enumerate(lower_answer):
            if char != w[i]:
                cnt_set[char] += 1
    ...
    def get_hint(self) -> str:
        ...
        f: bool = len(set(self.word) ^ self.hint_set) <= 1
        for i in self.word:
            if i in self.hint_set:
                back += i
            else:
                if f:
                    back += "*"
