...
from graia.ariadne.message.element import MusicShareKind, Voice, MusicShare


async def wyy_handle(
        keyword: str, send_type: Literal["voice", "card", "file"]
) -> Union[Voice, MusicShare, Tuple[str, bytes], None]:
    ...
    if data := data.get("result"):
        ...
        try:
            ...
            logger.warning(f"wyy点歌请求歌曲图片出现错误: {e}")
            picture_url = "https://p2.music.126.net/6y-UleORITEDbvrOLV0Q8A==/5639395138885805.jpg"
        if send_type == "card":
            return MusicShare(
